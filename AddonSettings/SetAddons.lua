...
local function SetCQL()
	if not IsAddOnLoaded('Classic Quest Log') then
		return
	end
	if ClassicQuestLogSettings then
		table.wipe(ClassicQuestLogSettings)
	end
...
local function SetLiteBag()
	...
	if LiteBag_OptionsDB then
		table.wipe(LiteBag_OptionsDB)
	end
	LiteBag_OptionsDB = {
