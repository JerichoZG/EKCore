...
local FONT_FACE,
	FONT_SIZE = STANDARD_TEXT_FONT, 18
...
local DAY,
	HOUR,
	MINUTE = 86400, 3600, 60
local DAYISH,
	HOURISH,
	MINUTEISH = 3600 * 23.5, 60 * 59.5, 59.5
local HALFDAYISH,
	HALFHOURISH,
	HALFMINUTEISH = DAY / 2 + 0.5, HOUR / 2 + 0.5, MINUTE / 2 + 0.5
...
local function Timer_Start(self, start, duration)
	...
	if self:GetParent():GetParent() == PVPQueueFrame then
		return
	end
	if (self.noOCC) then
		return
...
local function Cooldown_Update(self)
	...
	local start,
		duration,
		enable = GetActionCooldown(button.action)
